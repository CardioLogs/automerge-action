const { logger, retry, doSleep } = require("./common");
const { updateFromBranch } = require("./update");
...
async function merge(context, pullRequest) {
  ...
  if (merged) {
    ...
    await doSleep(30000);
    await updateFromBranch(context, pullRequest.base.repo.owner.login, pullRequest.base.repo.name, pullRequest.base.ref);
  }
