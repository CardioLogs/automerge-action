...
async function updateAllPullRequests(context, pullRequest) {
  ...
  logger.info("PRList:", prList);
  asyncForEach(prList, async (prInfo) => {
    const { data: fullPullRequest } = await octokit.pulls.get({
      owner: pullRequest.base.repo.owner.login,
      repo: pullRequest.base.repo.name,
      pull_number: prInfo.number
    });
    logger.info("FullPR:", fullPullRequest);
    await update(context, fullPullRequest);
  });
  return true;
}
...
module.exports = { update, updateAllPullRequests };
