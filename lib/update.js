const { logger, tmpdir, doSleep } = require("./common");
const git = require("./git");

const FETCH_TIMEOUT = 60000;

async function updateFromBranch(context, owner, repo, branch) {
  logger.debug("Updated branch:", branch);

  const { octokit } = context;

  // Sleep 30 seconds to ensure the merge status is updated
  await doSleep(30000);
...
function skipPullRequest(context, pullRequest) {
  ...
  if (pullRequest.state !== "open") {
    logger.info(`Skipping PR #${pullRequest.number} update, state is not open: ${pullRequest.state}`);
    skip = true;
  }

  if (pullRequest.merged === true) {
    logger.info(`Skipping PR #${pullRequest.number} update, already merged!`);
    skip = true;
  }
  ...
  for (const label of pullRequest.labels) {
    if (updateLabels.blocking.includes(label.name)) {
      logger.info(`Skipping PR #${pullRequest.number} update, blocking label present: ${label.name}`);
      skip = true;
    }
  }
  ...
  for (const required of updateLabels.required) {
    if (!labels.includes(required)) {
      logger.info(`Skipping PR #${pullRequest.number} update, required label missing:: ${required}`);
      skip = true;
    }
  }
...
async function merge(octokit, pullRequest) {
  ...
  if (mergeableState === "behind" || mergeableState === "unknown") {
...
module.exports = { update, updateFromBranch };
